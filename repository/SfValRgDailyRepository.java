...
@Repository
public interface  SfValRgDailyRepository extends JpaRepository<BillingSfvalRgDaily, Long> {
    ...
    @Query(value = "SELECT * FROM bill_sfval_rg_daily WHERE aid = :aid " +
            "AND month = :month " +
            "AND year = :year", nativeQuery = true)
    List<BillingSfvalRgDaily> findAllByAidAndMonthAndYear(
